...
import { YIELD_HOOK_ABI, ERC20_ABI, ERC4626_ABI } from '../abis'
...
export default function LPCard({ onPositionAdded }: { onPositionAdded: () => void }) {
  ...
  const liquidity: bigint =
    preview0 !== undefined && preview1 !== undefined
      ? preview0 < preview1
        ? preview0
        : preview1
      : 0n
  ...
  useEffect(() => {
    if (addSuccess && addReceipt) {
      onPositionAdded()
    }
  }, [addSuccess, addReceipt, onPositionAdded])
  ...
  function handleApprove0() {
    approve0({ address: USDC_ADDRESS, abi: ERC20_ABI, functionName: 'approve', args: [YIELD_HOOK_ADDRESS, maxUint256] })
  }

  function handleApprove1() {
    approve1({ address: USDT_ADDRESS, abi: ERC20_ABI, functionName: 'approve', args: [YIELD_HOOK_ADDRESS, maxUint256] })
  }
  ...
  function handleAddLiquidity() {
    ...
    addLiquidity({
      ...
      args: [parsed0, parsed1, TICK_LOWER, TICK_UPPER, liquidity as unknown as bigint],
    })
  }
    ...
    <div className="bg-white rounded-2xl shadow p-6 w-full max-w-md border border-gray-100">
      ...
      <div className="mb-3">
        ...
        {preview0 !== undefined && parsed0 > 0n && (
          <p className="text-xs text-gray-400 mt-1">≈ {preview0.toString()} stataUSDC shares</p>
        )}
      </div>
      ...
      <div className="mb-4">
        ...
        {preview1 !== undefined && parsed1 > 0n && (
          <p className="text-xs text-gray-400 mt-1">≈ {preview1.toString()} stataUSDT shares</p>
        )}
      </div>

      {liquidity > 0n && (
        <div className="mb-4 bg-indigo-50 rounded-xl px-4 py-2 text-sm text-indigo-700">
          Estimated liquidity: <strong>{liquidity.toString()}</strong>
        </div>
      )}
      ...
      {addTxHash && (
        <div className="mt-4 text-sm text-center">
          <a
            href={`https://sepolia.basescan.org/tx/${addTxHash}`}
            target="_blank"
            rel="noreferrer"
